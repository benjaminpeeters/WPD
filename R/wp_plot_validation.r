...
#' 
#' Used by: in_validate_bar_args, in_validate_series_args, in_validate_scatter_args
#' Uses: in_print_debug, wp_from_iso
#' 
#' Validation steps:
...
#'
#' @keywords internal
in_validate_common_args <- function(args) {                    # list: full set of function arguments to validate
    ...
    # in_print_debug("Starting wp_plot_scatter", verbose, debug, type = "info", "MAIN")
    ...
    for (param in logical_params) {
        if (!is.null(args[[param]]) && !is.logical(args[[param]])) {
            ...
            in_print_debug(sprintf("Parameter '%s' coerced to logical: %s", param, args[[param]]), 
    ...
    if (is.null(args$print)) {
        ...
        in_print_debug(sprintf("Parameter 'print' coerced to logical: %s", args$print), 
                   args$verbose, args$debug, type = "warning")
    }
    ...
    if(!"Country" %in% names(args$data)) {
        in_print_debug(sprintf("Column Country added to data"), args$verbose, args$debug, type = "info", "COUNTRY")
        args$data$Country <- wp_from_iso(args$data$ISO, opt = "name", verbose = FALSE)
    }
    ...
    if (isTRUE(args$reference)) {
        if (!"Reference" %in% names(args$data)) {
            in_print_debug("'Reference' column is missing but reference = TRUE. Setting reference = FALSE", 
                       args$verbose, args$debug, type = "warning")
            args$reference <- FALSE
        } else if (all(is.na(args$data$Reference))) {
            in_print_debug("'Reference' column contains only NA values. Setting reference = FALSE", 
                       args$verbose, args$debug, type = "warning")
            args$reference <- FALSE
        }
    }
    ...
    for (param in null_if_false) {
        if (!is.null(args[[param]]) && isFALSE(args[[param]])) {
            ...
            in_print_debug(sprintf("Parameter '%s' was FALSE, converted to NULL", param), 
                       args$verbose, args$debug, type = "info", "CONVERT")
        }
    }
    ...
    if (!is.null(args$size) && !args$size %in% c(1, 2, 3, 4)) {
        in_print_debug("Invalid 'size' parameter. Should be 1, 2, 3, or 4. Using default: 1", 
                   args$verbose, args$debug, type = "warning")
        args$size <- 1
    }
    ...
    if (is.null(base_size_value) || is.na(base_size_value) || base_size_value <= 0 || is.logical(base_size_value)) {
        in_print_debug("Invalid 'base_size' parameter. Using default: 16", 
                     args$verbose, args$debug, type = "warning")
        args$base_size <- 16
    } else {
    ...
    if (!is.null(args$key_dates)) {
        if (!(is.data.frame(args$key_dates) || is.matrix(args$key_dates))) {
            in_print_debug("key_dates must be a data frame or matrix. Setting to NULL", 
                       args$verbose, args$debug, type = "warning")
            args["key_dates"] <- list(NULL)
        } else if (ncol(args$key_dates) != 2) {
            in_print_debug("key_dates must have exactly 2 columns. Setting to NULL", 
                       args$verbose, args$debug, type = "warning")
            args["key_dates"] <- list(NULL)
        } 
    }
...
#' 
#' Used by: wp_plot_series
#' Uses: in_validate_common_args, in_print_debug
#' 
#' Validation:
...
#'
#' @keywords internal
in_validate_series_args <- function(args) {                    # list: full set of time series arguments to validate


    # First validate common arguments
    args <- in_validate_common_args(args)
    ...
    if (args$verbose) {
        msg <- paste0("n_countries = ", n_countries, 
                     ...
                     " | plot_type = series")
        in_print_debug(msg, args$verbose, args$debug, type = "info", "CONFIG")
    }
    ...
    if (!is.null(args$by_indicator)) {
        if (n_countries == 1 || n_indicators == 1) {
            in_print_debug("by_indicator parameter ignored for single country or single indicator", 
                       args$verbose, args$debug, type = "warning")
    ...
    if (!is.null(args$subfig_title)) {
        if (n_countries == 1 || n_indicators == 1) {
            in_print_debug("subfig_title parameter ignored for single country or single indicator", 
                       args$verbose, args$debug, type = "warning")
            ...
            in_print_debug(sprintf("Invalid subfig_title: %s. Setting to default (subfig_title = TRUE)", args$subfig_title),
                       args$verbose, args$debug, type = "warning")
    ...
    if (!is.null(args$right_axis)) {
        if (n_indicators < 2) {
            in_print_debug("right_axis requires at least 2 indicators. Setting right_axis = NULL", 
                       args$verbose, args$debug, type = "warning")
            args$right_axis <- NULL
        }else if (!is.character(args$right_axis) && !isTRUE(args$right_axis)) {
            in_print_debug("Invalid right_axis parameter. Must be TRUE or character. Setting to TRUE", 
                       args$verbose, args$debug, type = "warning")
            ...
            if (length(missing_indicators) > 0) {
                in_print_debug(sprintf("Indicators not found for right_axis: %s. Setting to TRUE", 
    ...
    if (isTRUE(args$area) && isTRUE(args$right_axis)) {
        in_print_debug("Parameters 'area' and 'right_axis' cannot be used together. Setting right_axis = NULL", 
                   args$verbose, args$debug, type = "warning")
...
#'
#' @keywords internal
in_validate_bar_args <- function(args) {                    # list: full set of bar plot arguments to validate
    ...
    if (!is.null(args$type) && !args$type %in% c("Mean", "Last")) {
        in_print_debug("Invalid 'type' parameter. Using default: 'Mean'", 
                   args$verbose, args$debug, type = "warning")
        args$type <- "Mean"
    }
    ...
    if (!is.null(args$show_values) && !args$show_values %in% c("Bottom", "Top", FALSE)) {
        in_print_debug("Invalid 'show_values' parameter. Using default: 'Bottom'", 
                   args$verbose, args$debug, type = "warning")
    ...
    if (!is.null(args$show_year) && !args$show_year %in% c(0, 1, 2)) {
        in_print_debug("Invalid 'show_year' parameter. Using default: 1", 
                   args$verbose, args$debug, type = "warning")
        args$show_year <- 1
    }
    ...
    if (!is.null(args$order)) {
        if (is.logical(args$order)) {
            if (is.na(args$order)) {
                in_print_debug("NA value in order parameter. Using default: 'Variable'", 
                          args$verbose, args$debug, type = "warning")
                args$order <- "Variable"
            } else {
            ...
            if (args$order < 1) {
                in_print_debug("Order value less than 1. Using order = 1", 
                          args$verbose, args$debug, type = "warning")
                args$order <- 1
            }
        } else if (!args$order %in% c("Alphabetical", "Variable")) {
            in_print_debug("Invalid 'order' parameter. Using default: 'Variable'", 
                       args$verbose, args$debug, type = "warning")
            args$order <- "Variable"
        }
    } else {
    ...
    if (!is.null(args$color)) {
        if (n_indicators > 1) {
            ...
            if (isTRUE(args$color)) {
                ...
                args$color <- "Variable"
                in_print_debug("Setting color to 'Variable' for multi-indicator bar plot", 
                          args$verbose, args$debug, type = "warning")
            }
        } else {
            ...
            if (isTRUE(args$color)) {
                ...
                args$color <- "Subregion"
                in_print_debug("Setting color to 'Subregion' for single-indicator bar plot", 
                          args$verbose, args$debug, type = "warning")
            }
        }
        ...
        if (!args$color %in% c("Subregion", "Hydrocarbon", "Variable", "Region", "Center-Periphery", FALSE)) {
            in_print_debug("Invalid 'color' parameter. Using default: NULL", 
                       args$verbose, args$debug, type = "warning")
...
#' 
#' Used by: wp_plot_scatter
#' Uses: in_validate_common_args, in_validate_color_list, in_print_debug
#' 
#' Validation:
...
#'
#' @keywords internal
in_validate_scatter_args <- function(args) {                    # list: full set of scatter plot arguments to validate
    ...
    if (!is.null(args$subfig_title)) {
        if (n_indicators < 2) {
            in_print_debug("subfig_title parameter ignored for two indicators", 
                       args$verbose, args$debug, type = "warning")
            args$subfig_title <- NULL
        } else if (is.character(args$subfig_title)) {
            N <- length(args$subfig_title)
    ...
    if (!is.null(args$ISO) && !args$ISO %in% c(TRUE, "Both", FALSE)) {
        in_print_debug("Invalid 'ISO' parameter. Using default: NULL", 
                   args$verbose, args$debug, type = "warning")
        args$ISO <- NULL
    }
    ...
    if (!is.null(args$interpolation)) {
        if (isTRUE(args$interpolation)) {
            ...
            in_print_debug("Invalid 'interpolation' parameter. Using default: FALSE", 
                       args$verbose, args$debug, type = "warning")
    ...
    if (!is.null(args$spline_bw)) {
        ...
        if (is.na(args$spline_bw) || args$spline_bw <= 0) {
            in_print_debug("Invalid 'spline_bw' parameter. Using default: NULL", 
                       args$verbose, args$debug, type = "warning")
            args$spline_bw <- NULL
        }
    }
    ...
    if (!is.null(args$r_squared) && !args$r_squared %in% c(0, 1, 2, 3)) {
        in_print_debug("Invalid 'r_squared' parameter. Using default: 0", 
                   args$verbose, args$debug, type = "warning")
        args$r_squared <- 0
    }
    ...
    if (!is.null(args$r_squared_pos) && !args$r_squared_pos %in% valid_positions) {
        in_print_debug("Invalid 'r_squared_pos' parameter. Using default: 'flexible'", 
                   args$verbose, args$debug, type = "warning")
    ...
    if (!is.null(args$label_nudge)) {
        ...
        if (is.na(args$label_nudge) || args$label_nudge < 0) {
            in_print_debug("Invalid 'label_nudge' parameter. Using default: 0.05", 
                       args$verbose, args$debug, type = "warning")
    ...
    if (!is.null(args$color)) {
        if (is.list(args$color)) {
            in_print_debug("Validating color list", args$verbose, args$debug, type = "info", "COLOR")
            ...
            for (group_name in names(args$color)) {
                ...
                if (length(invalid_countries) > 0) {
                    ...
                    in_print_debug(warning_msg, args$verbose, args$debug, type = "warning", "COLOR")
                }
            ...
            in_print_debug("Invalid 'color' parameter. Using default: 'Subregion'", 
                       args$verbose, args$debug, type = "warning")
            args$color <- "Subregion"
        }
    }
    ...
    if (!is.null(args$common_var)) {
        if (is.character(args$common_var)) {
            ...
            in_print_debug("Invalid 'common_var' parameter. Using default: TRUE", 
                       args$verbose, args$debug, type = "warning")
...
#' 
#' Used by: in_validate_scatter_args
#' Uses: in_print_debug
#' 
#' Validation:
...
#'
#' @keywords internal
# in_validate_color_list <- function(color_list,            # list: custom color grouping list
...
#
#
#     in_print_debug("Starting color list validation", verbose, debug, type = "info", "COLOR")
#     
#     # Get unique countries in data
...
#                                  group_name, 
#                                  paste(invalid_countries, collapse = ", "))
#             in_print_debug(warning_msg, verbose, debug, type = "warning", "COLOR")
#         }
#         
#         # Check for duplicate countries across groups
