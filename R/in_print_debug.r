...
#' @keywords internal
in_print_debug <- function(text, verbose = TRUE, debug = FALSE, type = "info", text_type = "INFO") {
    # Only print if either verbose or debug is TRUE
    if (!verbose && !debug) return(NULL)
    
    # Define color codes
    colors <- list(
        info = in_create_color("cyan"),
        info_loop = in_create_color("green"), 
        debug = in_create_color("white"), 
        wrapper = in_create_color("purple", 2),
        warning = in_create_color("red"),
        missing = in_create_color("yellow"),
        empty = in_create_color("blue", "bold")
        )
...
#'
#' @keywords internal
in_create_color <- function(color, style = "normal") {
    # Basic color codes
    color_codes <- list(
        black = 30,
        red = 31,
