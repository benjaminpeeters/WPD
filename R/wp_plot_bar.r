...
wp_plot_bar <- function(data,                    # data.frame with required columns: ISO, Date, Variable, Value
    ...
    # Validate args
    validated_args <- in_validate_bar_args(args)
...
#' 
#' Used by: wp_plot_bar (via in_plot_core)
#' Uses: in_process_bar_data, in_create_bar_labels, in_create_color_mapping,
#'       in_calculate_plot_limits, in_create_bar_legend, in_theme_plot
#' 
...
#'
#' @keywords internal
in_plot_one_indic_bar <- function(data,                    # data.frame: input data for single indicator
                                 y_axis = NULL,            # string/NULL: y-axis label
                                 type = "Mean",            # string: "Mean"/"Last" - determines value to display
                                 order = "Variable",       # string/numeric: "Variable"/"Alphabetical" - controls bar ordering
    ...
    # 1. Process data
    all_data <- in_process_bar_data(data, type, order)
    
    # 2. Create labels
    all_data <- in_create_bar_labels(all_data, show_year, type)
    
    # 3. Handle colors
    color_result <- in_create_color_mapping(all_data, color, base_size)
    ...
    # 9. Add color legend
    if(!is.null(color) && !isFALSE(color)) {
        category_labels <- in_get_category_labels()
...
#' 
#' Used by: in_plot_one_indic_bar
#' Uses: in_calculate_bar_statistics
#' 
#' Processing steps:
...
#'
#' @keywords internal
in_process_bar_data <- function(data,                    # data.frame: raw input data
    ...
    all_data <- in_calculate_bar_statistics(data, type)
...
#' 
#' Used by: in_plot_one_indic_bar, in_plot_multi_indic_bar
...
#'
#' @keywords internal
in_create_bar_labels <- function(data,                    # data.frame: data containing values to label
...
#'
#' @keywords internal
in_calculate_plot_limits <- function(data,                    # data.frame: data containing values to plot
...
#'
#' @keywords internal
in_create_bar_legend <- function(data,                    # data.frame: data for legend creation
...
#' 
#' Used by: in_process_bar_data, in_process_multi_bar_data
#' Uses: No direct function calls
#' 
...
#'
#' @keywords internal
in_calculate_bar_statistics <- function(data,                    # data.frame: input data to calculate statistics from
...
#' 
#' Used by: in_plot_one_indic_bar, in_plot_multi_indic_bar
#' Uses: in_get_color_group, in_get_category_labels, in_get_color_palettes
#' 
#' Features:
...
#'
#' @keywords internal
in_create_color_mapping <- function(data,                    # data.frame: data to create mapping for
    ...
    category_labels <- in_get_category_labels()
    color_palettes <- in_get_color_palettes()
    
    # Add color grouping to data
    data$color_group <- sapply(data$ISO, function(iso) in_get_color_group(iso, color))
...
#' 
#' Used by: in_create_color_mapping
#' Uses: No direct function calls
#' 
#' Features:
...
#'
#' @keywords internal
in_get_ordered_groups <- function(color_type,              # string: type of color scheme being used
...
#' 
#' Used by: wp_plot_bar (via in_plot_core)
#' Uses: in_process_multi_bar_data, in_calculate_plot_limits, in_theme_plot,
#'       in_get_legend_columns
#' 
#' Features:
...
#'
#' @keywords internal
in_plot_multi_indic_bar <- function(data,                    # data.frame: input data for multiple indicators
                                   y_axis = NULL,            # string/NULL: y-axis label
                                   type = "Mean",            # string: "Mean"/"Last" - determines value to display
    ...
    in_print_debug("Starting multi-indicator bar plot", verbose, debug, type = "info", "MAIN")
    
    # 1. Process data
    all_data <- in_process_multi_bar_data(data, type, order, verbose, debug)
    ...
    in_print_debug(paste("Bar positioning:", 
                     "width =", bar_width,
                     "space =", group_space,
                     "dodge =", position_dodge_width), 
               verbose, debug, type = "info", "POSITION")
    
    # 3. Handle colors
    if(is.null(color) || isFALSE(color)) {
        # Use gray scales for variables
        color_mapping <- in_create_variable_darkness(variables, verbose = verbose, debug = debug)
    } else {
    ...
    in_print_debug("Color mapping created", verbose, debug, type = "info", "COLORS")
...
#' 
#' Used by: in_plot_multi_indic_bar
#' Uses: No direct function calls
#' 
#' Features:
...
#'
#' @keywords internal
in_create_variable_darkness <- function(variables,             # character vector: variables to create scale for
    ...
    if(verbose) {
        ...
        in_print_debug(msg, verbose, debug, type = "info", "COLORS")
    }
...
#' 
#' Used by: in_plot_multi_indic_bar
#' Uses: in_calculate_bar_statistics, in_print_debug
#' 
#' Features:
...
#'
#' @keywords internal
in_process_multi_bar_data <- function(data,                    # data.frame: raw input data for multiple indicators
    ...
    if(verbose) {
        in_print_debug("Starting data processing", verbose, debug, type = "info", "DATA")
    }
    ...
    if(verbose) {
        in_print_debug(paste("Variables found:", paste(variables, collapse = ", ")), 
    ...
    all_stats <- lapply(variables, function(var) {
        ...
        stats <- in_calculate_bar_statistics(var_data, type)
        stats$Variable <- var
        return(stats)
    })
    ...
    if (is.numeric(order)) {
        ...
        if(verbose) {
            in_print_debug(paste("Ordering by variable", var_index, ":", variables[var_index]), 
        ...
        if(verbose) {
            in_print_debug("Ordering by first variable values", verbose, debug, type = "info", "ORDER")
        }
        ...
        if(verbose) {
            in_print_debug("Ordering alphabetically", verbose, debug, type = "info", "ORDER")
        }
        country_order <- sort(unique(all_data$Country))
    }
    
    if(verbose) {
        in_print_debug(paste("Country order:", paste(country_order, collapse = ", ")), 
                   verbose, debug, type = "info", "ORDER")
    }
    ...
    if(verbose) {
        in_print_debug(paste("Final data rows:", nrow(all_data)), 
                   verbose, debug, type = "info", "DATA")
        in_print_debug(paste("Final country order:", 
...
#'
#' @keywords internal
in_create_multi_bar_legend <- function(data,                    # data.frame: data for legend creation
    ...
    if(verbose) {
        in_print_debug("Creating multi-bar legend", verbose, debug, type = "info", "LEGEND")
    }
    ...
    if(verbose) {
        in_print_debug(paste("Statistical elements:", paste(stat_elements, collapse = ", ")), 
