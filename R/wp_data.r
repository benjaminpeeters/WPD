...
#' @export
wp_data <- function(ISO,                           # ISO3 codes or categories
    ...
    has_match <- any(sapply(list_categories, function(cat) {grepl(cat, ISO)}))

    if (has_match) {
        in_print_debug("Get ISO codes for ISO codes.", verbose, debug, "info", "Step 0")
        ISO <- wp_get_category(ISO)
        msg <- paste0("ISO3(",length(ISO),"): " , paste0(ISO, collapse = " "))
        in_print_debug(msg, verbose, debug, "info_loop")
    }


    ###############
    # STEP 1: Input Validation
    in_print_debug("Input Validation.", verbose, debug, "info", "Step 1")
    if (is.null(variable)){ variable <- formula }
    in_validate_inputs(ISO, formula, variable, years, adjust_seasonal)
    ...
    in_print_debug("Data Filtering (ISO codes, Symbols, and Years).", verbose, debug, "info", "Step 2")
    filtered_data <- in_load_filter_data(ISO, years, formula, verbose) 
    ...
    in_print_debug("Loop through each formula.", verbose, debug, "info", "Steps 3 to 7")
    for (i in seq_along(formula)) {
        form <- formula[i]
        var <- variable[i]

        # Call in_data_one_formula for each formula
        result <- in_data_one_formula(
    ...
    if (!is.null(aggregate_iso)) {
        ...
        in_print_debug(msg, verbose, debug, "info", "Step 8")
        combined_results <- in_aggregate_countries(combined_results, 
    ...
    if (!is.null(aggregate_period)) {
        ...
        in_print_debug(msg, verbose, debug, "info", "Step 8")
        combined_results <- in_aggregate_time(combined_results, 
    ...
    if (adjust_seasonal && is.null(aggregate_period)) {
        ...
        in_print_debug(msg, verbose, debug, "info", "Step 9")
        V <- NULL
        if (is.null(variable)) {variable <- formula}
        for (v in variable){
            ...
            if( nchar(combined_results[combined_results$Variable == v,]$Date[1]) == 6 ){
                V <- c(V,v)
                in_print_debug(paste0("Seasonal adjustment for: ", v), FALSE, debug, "debug")
                head(combined_results)
                combined_results[combined_results$Variable == v,] <- in_adjust_seasonal(combined_results[combined_results$Variable == v,],
                                                                                        window_seasadj = window_seasadj)
                head(combined_results)
            }else{
                in_print_debug(paste0("No seasonal adjustment for: ", v), FALSE, debug, "debug")
            }
        }
        msg <- paste0("Seasonal adjustments for: ", paste0(V, collapse = "   "))
        in_print_debug(msg, verbose, FALSE, "info")
    }

    ###############
    # STEP 10: CLEAN (remove NAs)
    if (clean) {
        in_print_debug(paste0("Clean database [remove NAs in output] - clean is ",clean,"."), verbose, debug, "info", "Step 10")
    ...
    # Add Country names
    if (country_names) {
        combined_results$Country <- wp_from_iso(combined_results$ISO, opt = "name")
    }
...
#'
#' Used by: wp_data
#' Uses: in_extract_symbols, in_determine_symbol_frequencies, in_check_missing_data,
#'       in_adjust_columns_to_frequency, in_create_symbol_environment, 
#'       in_create_country_result
#'
#' Features:
...
#'
#' @keywords internal
in_data_one_formula <- function(countries,                  # ISO3 codes
    ...
    symbols <- in_extract_symbols(formula)
    
    msg <- paste0("Step 3: Symbols: ", paste0(symbols, "  ", collapse = " "))
    in_print_debug(msg, FALSE, debug, "debug")
    ...
    frequencies <- in_determine_symbol_frequencies(data, symbols)
    ...
    in_print_debug(msg, verbose, FALSE, "info_loop")


    # Check for missing data (consolidated check)
    in_check_missing_data(data, countries, symbols, formula, years, verbose)


    ###############
    # STEP 5: Adjust to frequency
    in_print_debug("Step 5: Adjust frequencies.", verbose, debug, "info_loop")

    data <- in_adjust_columns_to_frequency(data = data, 
    ...
    in_print_debug("Step 6-7: Data Processing (Loop for each country).", verbose, debug, "info_loop")
    for (country in countries) {
        in_print_debug(paste0("Step 6.0: Country: ", country), FALSE, debug, "debug")
        country_data <- data[data$ISO == country, ]
        
        in_print_debug("Step 6.1: Create an environment for each formula.", FALSE, debug, "debug")
        # STEP 6
        # Create an environment with the data for each symbol
        env <- in_create_symbol_environment(country_data, symbols, date_cols)
        
        in_print_debug("Step 6.2: Evaluate the formula.", FALSE, debug, "debug")
        # Evaluate the formula
        result <- eval(parse(text = formula), envir = env)


        # Check if result is empty or has zero length
        # if (length(result) == 0 || all(is.na(result)) ) {
        #     in_print_debug(paste("No data for country:", country), verbose, debug, "info_loop", )
        #     next
        # }
    ...
    in_print_debug("", verbose, debug, "info_loop")
...
#'
#' @keywords internal
in_aggregate_countries <- function(df,                     # input dataframe
    ...
    for (d in unique(subset_df$Date)) {
      ...
      if (method == "Sum") {
        result <- data.frame(
          ...
          Reference = date_data$Reference[1]
        )
...
#'
#' @keywords internal
in_aggregate_time <- function(df,                         # input dataframe
        ...
        iso_results <- lapply(iso_groups, function(iso_data) {
            ...
            if (method == "Growth") {
                ...
                if (length(non_na_values) < 3) {
                    ...
                    in_print_debug(warning_msg, TRUE, FALSE, "warning")
                    agg_value <- NA
                } else {
                    ...
                    if (any(values < 0, na.rm = TRUE)) {
                        ...
                        growth_rates <- calculate_yoy_growth(iso_data)
                        if (length(growth_rates) > 0) {
                            agg_value <- geometric_mean(growth_rates) * 100  # Convert back to percentage
                        } else {
                            agg_value <- NA
                        }
                    }
                ...
                if (any(values < -1, na.rm = TRUE)) {  # Check if any value < -100%
                    ...
                    agg_value <- geometric_mean(values)*100   # Convert back to percentage
                }
            } else {
            ...
            base_result <- data.frame(
                ...
                Value = agg_value,
                Reference = paste(unique(sort(unlist(strsplit(iso_data$Reference, "; ")))), collapse = "; "),
            ...
            if (quartile && !method %in% c("Sum", "SD")) {
                if (method %in% c("Growth", "CAGR")) {
                    growth_rates <- calculate_yoy_growth(iso_data) * 100  # Use actual growth rates for quartiles
...
#'
#' Features:
#' - Handles both quarterly and annual data automatically
#' - Preserves quarter-to-quarter comparisons in quarterly data
#' - Skips missing values in calculations
#' - Handles zero base values gracefully
#' 
#' @param data data.frame; input data with Date and Value columns
#'
#' @return numeric vector of growth rates
#'
#' @keywords internal
calculate_yoy_growth <- function(data) {                  # input data with Date and Value columns
...
#' 
#' @description
#' Internal helper function that computes geometric mean with proper handling
#' of negative values and zeroes.
#'
#' Used by: in_aggregate_time
#' Uses: None
#'
...
#' - Uses log transformation for computation
#' - Returns NA for empty or all-NA inputs
#' - Adjusts values by +1 before calculation
#'
#' @param x numeric vector; values to average
#'
#' @return numeric; geometric mean of input values
#'
#' @keywords internal
geometric_mean <- function(x) {                          # numeric vector
...
# STEP 7 - Create the good structure with the results (from the evaluation of the formula)


#' Create Result Structure for Country Data
#' 
#' @description
...
#' @param result numeric; calculated values
#'
#' @return data.frame with standardized structure
#'
#' @keywords internal
in_create_country_result <- function(country_data,       # source country data
...
#'
#' @keywords internal
in_create_symbol_environment <- function(country_data,    # source country data
    ...
    for (sym in symbols) {
        ...
        if (nrow(sym_data) > 0) {
            ...
            # warning(paste("[in_create_symbol_environment] No data found for symbol:", sym, "in country:", country_data$ISO[1]))
...
#'
#' Used by: in_data_one_formula
#' Uses: in_process_yearly_data, in_match_Q2Y, in_match_Y2Q
#'
#' Features:
...
#'
#' @keywords internal
in_adjust_columns_to_frequency <- function(data,                    # input dataset
    ...
    if (all(frequencies == "Q")) {
        ...
        data <- in_process_yearly_data(data)
    } else {
        ...
        in_print_debug("Mixed frequencies detected.", verbose, debug, type = "info_loop")

        matching_yq = "Y2Q"
        if(matching_yq == "Q2Y"){
            data <- in_match_Q2Y(data, frequencies, symbols, verbose, debug)
        }else if(matching_yq == "Y2Q"){
            data <- in_match_Y2Q(data, frequencies, symbols, interpolation_method = interpolation_method, verbose, debug)
        }
...
#'
#' @keywords internal
in_process_yearly_data <- function(symbol_data) {        # yearly data to process
...
#'
#' @keywords internal
in_match_Q2Y <- function(data,                           # input dataset
                        frequencies,                      # "Q" or "Y" for each symbol
                        symbols,                          # symbols to process
                        verbose = TRUE,                   # TRUE/FALSE
                        debug = FALSE) {                  # TRUE/FALSE

    in_print_debug("Using in_match_Q2Y to match frequencies: Adjusting data quarterly data to yearly frequency.", verbose, debug, type = "info_loop")
    ...
    for (i in seq_along(symbols)) {
        ...
        if (freq == "Y") {
            symbol_data <- in_process_yearly_data(symbol_data)
            in_print_debug(paste("Symbol", symbol, "already in yearly frequency."), verbose, debug, type = "info_loop")
        } else if (freq == "Q") {
            ...
            symbol_data <- in_process_yearly_data(symbol_data)
            in_print_debug(paste("Symbol", symbol, "converted from quarterly to yearly frequency."), verbose, debug, type = "info_loop")
        } else {
...
#'
#' Used by: in_adjust_columns_to_frequency
#' Uses: in_linear_interpolate_quarterly
#'
#' Features:
...
#'
#' @keywords internal
in_match_Y2Q <- function(data,                           # input dataset
                        frequencies,                      # "Q" or "Y" for each symbol
                        symbols,                          # symbols to process
                        interpolation_method = "Linear",  # "None", "Linear", "Linear-Scale"
                        verbose = TRUE,                   # TRUE/FALSE
    ...
    in_print_debug("Using in_match_Y2Q to match frequencies: Adjusting yearly data to quarterly frequency.", verbose, debug, type = "info_loop")
    # cat("\n\n===========================================================================\n\n")
    # cat("Date input of in_match_Y2Q:\n")
    # print(dim(data))
    # print(data)
    # cat("\n\n===========================================================================\n\n")
    list_symbol_data <- list()
    ...
    for (i in seq_along(symbols)) {
        ...
        if (freq == "Q") {
            in_print_debug(paste("Symbol", symbol, "already in quartely frequency."), verbose, debug, type = "info_loop")
        } else if (freq == "Y") {
            ...
            if (interpolation_method != "None") {
                ...
                if (interpolation_method == "Linear") {
                    interpolation_data <- in_linear_interpolate_quarterly(interpolation_data, scale = FALSE)
                } else if (interpolation_method == "Linear-Scale") {
                    interpolation_data <- in_linear_interpolate_quarterly(interpolation_data, scale = TRUE)
                } else {
            ...
            in_print_debug(paste("Symbol", symbol, "converted from yearly to quarterly frequency using", interpolation_method, "interpolation"), verbose, debug, type = "info_loop")
        } else {
            stop(paste("Unsupported frequency", freq, "for symbol", symbol))
        }
...
#'
#' @keywords internal
in_linear_interpolate_quarterly <- function(data,          # input data matrix
...
#'
#' Used by: in_linear_interpolate_quarterly
#' Uses: None
#'
#' Features:
...
#'
#' @keywords internal
in_determine_symbol_frequencies <- function(data,        # input dataset
...
#' using regex.
#'
#' Used by: in_data_one_formula, in_load_filter_data
#' Uses: None
#'
#' Features:
...
#'
#' @keywords internal
in_extract_symbols <- function(formula) {                # mathematical expression
...
#'
#' Used by: wp_data
#' Uses: in_extract_symbols, in_load_frequency_data, in_filter_data_by_frequency, in_merge_yearly_quarterly
#'
#' Features:
...
#'
#' @keywords internal
in_load_filter_data <- function(countries,              # ISO codes
                               years,                    # c(start_year, end_year)
                               formula,                  # mathematical expression
                               verbose) {                # TRUE/FALSE
    ...
    all_symbols <- unique(in_extract_symbols(formula))

    # Determine which databases contain our symbols
    required_dbs <- in_locate_symbol(all_symbols)
   ...
   # Initialize data variables
    ...
    if ("Q" %in% required_dbs) {
        in_load_frequency_data(frequency = "Q", 
                               verbose = verbose)
        DATA_Q <- .pkgenv[["DATA_Q"]]
        data_q <- in_filter_data_by_frequency(countries, years, DATA_Q, formula, frequency = 'Q')
    }
    
    if ("Y" %in% required_dbs) {
        in_load_frequency_data(frequency = "Y",
                               verbose = verbose)
        DATA_Y <- .pkgenv[["DATA_Y"]]
        data_y <- in_filter_data_by_frequency(countries, years, DATA_Y, formula, frequency = 'Y')
    }
    
    # Determine final dataset based on what was loaded
    if (is.null(data_y)) {
        data <- data_q
    } else if (is.null(data_q)) {
        data <- data_y
    } else {
        data <- in_merge_yearly_quarterly(data_y, data_q)
    }
...
#'
#' @keywords internal
in_filter_data_by_frequency <- function(countries,      # ISO codes
                                      years,            # c(start_year, end_year)
                                      data,             # input dataset
                                      formula,          # mathematical expression
                                      frequency) {      # "Q" or "Y"
...
#'
#' @keywords internal
in_merge_yearly_quarterly <- function(DATA_Y,           # yearly dataset
...
#'
#' @keywords internal
in_validate_inputs <- function(countries,               # ISO codes
                             formula,                   # mathematical expressions
...
#'
#' @keywords internal
in_check_missing_data <- function(data,                 # input dataset
                                countries,               # ISO codes
                                symbols,                 # symbols to check
                                formula,                 # mathematical expression
                                years,                   # year range
    ...
    for (symbol in symbols) {
        ...
        for (country in countries) {
            ...
            if (length(years_with_values) == 0) {
                ...
                year_ranges <- in_create_year_ranges(years_with_values, all_years)
        ...
        if (length(missing_data_msgs) > 0) {
            ...
            # warning(msg)
            in_print_debug(msg, verbose = verbose, debug = FALSE, type = "missing")
...
#'
#' @keywords internal
in_create_year_ranges <- function(years_with_data,      # years with data
