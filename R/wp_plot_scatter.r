...
#' @export
wp_plot_scatter <- function(data,                    # data.frame with required columns: ISO, Variable, Value
    ...
    # Validate args  
    validated_args <- in_validate_scatter_args(args)
...
#' 
#' Used by: wp_plot_scatter (via in_plot_core)
#' Uses: in_prepare_scatter_data, in_calculate_r_squared, in_scale_y, in_theme_plot,
#'       in_create_scatter_colors
#' 
#' Features:
...
#'
#' @keywords internal
in_plot_scatter_two_vars <- function(data,                    # data.frame: input data for two variables
    ...
    in_print_debug("Starting two-variable scatter plot", verbose, debug, type = "info", "SCATTER")
    
    # 1. Data Preparation
    data_prep <- in_prepare_scatter_data(data, verbose, debug)
    ...
    if (!is.null(color)) {
        ...
        if (is.list(color)) {
            ...
            color_palettes <- in_get_color_palettes(original_color)
            category_labels <- in_get_category_labels(original_color)
            
        } else {
            # Original code for non-list color schemes
            plot_data$color_group <- sapply(plot_data$ISO, function(iso) in_get_color_group(iso, color))
        ...
        if(verbose) {
            in_print_debug(paste("Labels created:", paste(names(labels), labels, collapse="; ")), 
...
#'
#' @keywords internal
in_plot_scatter_multi_vars <- function(data,                    # data.frame: input data for multiple variables
                                     ...
                                     base_size = 11,           # numeric: base font size in points
    ...
    # Debug mode setup
    in_print_debug <- function(msg, type = "info") {
        if (debug) {
            if (type == "warning") {
    ...
    if (is.character(common_var)) {
        ...
        if (verbose) {
            in_print_debug(sprintf("Using first variable '%s' as common variable", x_var))
        }
    } else {
        # Sequential pairing (original same_var = FALSE behavior)
        if (length(variables) %% 2 != 0) {
            in_print_debug(paste("Last variable", variables[length(variables)], 
        ...
        if (verbose) {
            in_print_debug("Using sequential variable pairing")
        }
    }
...
#' 
#' Used by: in_plot_scatter_two_vars
...
#'
#' @keywords internal
in_prepare_scatter_data <- function(data,                    # data.frame: long format input data
                                   verbose = TRUE,           # logical: controls information messages
                                   debug = FALSE) {          # logical: controls debug messages

    in_print_debug("Preparing data for scatter plot", verbose, debug, type = "info", "DATA")
    ...
    in_print_debug(paste("Variables:", paste(variables, collapse = " vs ")), 
...
#' 
#' Used by: in_plot_scatter_two_vars
#' Uses: in_get_color_group, in_get_color_palettes
#' 
#' Features:
...
#'
#' @keywords internal
in_create_scatter_colors <- function(data,                    # data.frame: data to create mapping for
                                   color,                     # string/list/NULL: color scheme to use
                                   verbose = TRUE,            # logical: controls information messages
                                   debug = FALSE) {           # logical: controls debug messages


    in_print_debug("Creating color mapping", verbose, debug, type = "info", "COLOR")
    
    if (is.null(color) || isFALSE(color)) {
        in_print_debug("No color mapping requested", verbose, debug, type = "info", "COLOR")
        return(NULL)
    }
    
    if (is.list(color)) {
        in_print_debug("Creating custom color groups", verbose, debug, type = "info", "COLOR")
        # Custom color groups
        color_mapping <- rep("gray50", nrow(data))  # Default color
        custom_colors <- c("#4472C4", "#70AD47", "#C00000", "#FFC000", "#7030A0")
        
        for (i in seq_along(color)) {
            group_name <- names(color)[i]
            countries <- color[[i]]
            if(verbose) {
                in_print_debug(paste("Group:", group_name, "Countries:", 
                                paste(countries, collapse=", ")), 
                           verbose, debug, type = "info", "COLOR")
            }
            color_mapping[data$Country %in% countries] <- custom_colors[i]
        }
        
        if(verbose) {
            in_print_debug(paste("Unique colors in mapping:", 
        ...
        in_print_debug(paste("Using predefined color scheme:", color), 
                   verbose, debug, type = "info", "COLOR")
        # Use existing color mapping functions
        color_group <- sapply(data$ISO, function(iso) in_get_color_group(iso, color))
        
        if(verbose) {
            in_print_debug(paste("Unique color groups:", 
        ...
        if(verbose) {
            in_print_debug(paste("Unique colors after mapping:", 
                            paste(unique(color_mapping), collapse=", ")), 
                       verbose, debug, type = "info", "COLOR")
        }
...
#' 
#' Used by: in_calculate_r_squared
#' Uses: No direct function calls
#' 
#' Formatting rules:
...
#'
#' @keywords internal
in_format_coef <- function(x) {                    # numeric: coefficient to format
...
#'
#' @keywords internal
in_calculate_r_squared <- function(x,                          # numeric vector: x coordinates
    ...
    if (degree == 1) {
        ...
        model <- lm(y ~ x)
        coef <- coef(model)
        slope <- in_format_coef(coef[2])
        intercept <- in_format_coef(coef[1])
        ...
        coef <- coef(model)
        a <- in_format_coef(coef[3])
        b <- in_format_coef(coef[2])
        ...
        coef <- coef(model)
        a <- in_format_coef(coef[4])
        b <- in_format_coef(coef[3])
        c <- in_format_coef(coef[2])
